...
The v1 runtime classifies every observed failure into one of the five modes documented below. The classifier is `src/fail/catalog.py`; per-mode trigger fixtures live under `tasks/failure_modes/`; per-mode tests live under `tests/failure_modes/`.
...
- **Trace signature** — `retry_attempt` span with `agent.retry.outcome=exhausted`; the parent `agent_step` carries `agent.failure_mode=retry_exhaustion`. The agent's `terminal_reason="retry_exhausted"`.
...
The per-mode test suite at `tests/failure_modes/` verifies each mode end-to-end against the canonical `policy/v1.yaml`. Captured trace artifacts under `runs/failure_modes/<mode>/{trace.json, state.jsonl, run_report.md, manifest.json}` ship in a subsequent slice; this commit lands the catalog, the classifier, and the per-mode tests, not the captured runs.
